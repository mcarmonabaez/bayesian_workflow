...
load("data/marco_nal_2018.rda")
muestra <- read_delim("data/REMESAS0100012200.txt", delim = "|", skip = 1) %>% 
  dplyr::select(edo_id = iD_ESTADO, total_votos = TOTAL)

muestra <- read_delim("data/REMESAS0100012200.txt", delim = "|", skip = 1)
muestra_pt <- muestra %>% select(iD_ESTADO, ID_ESTRATO_L, LISTA_NOMINAL, TOTAL)
load("data/marco_nal_2018.rda")
marco <- marco_nal_2018 %>% select(id, casilla_id, estrato, ln, 
                                   edo_id = id_estado) %>% 
  left_join(regiones)
...
muestra_part <- muestra %>%  
        ...
        dplyr::left_join(marco) 

# distribución de la muestra por región
dist <- marco %>% 
  group_by(region) %>%
  tally %>% 
  left_join(muestra_part 
            %>% group_by(region) %>% 
              summarise(n_muestra = n())) %>% 
  mutate(prop = n_muestra/n)
dist

marco_noreste <- marco %>% 
  filter(region == 'Noreste') 

muestra_part_noreste <- muestra_part %>% 
  filter(region == 'Noreste')

aux <- data.frame(estrato = unique(muestra_part_noreste$estrato),
                  estrato_aux = 1:length(unique(muestra_part_noreste$estrato)))

muestra_part_noreste <- muestra_part_noreste %>% 
  left_join(aux)
marco_noreste <- marco_noreste %>% 
  left_join(aux)
...
# resumen distritos para elecciones de 2012
...
```{r}
iter <- 100
sim_data <- list(k = length(unique(marco_noreste$estrato)),
                 N = nrow(marco_noreste), est = marco_noreste$estrato_aux, 
                 votantes_nom = marco_noreste$ln)
...
dat <- as_tibble(t(sim_y[1:15,]), .name_repair = "unique") %>% 
  mutate(id = 1:21079) %>% gather(rep, votos, - id)
...
ajustar_sims <- function(iteracion, modelo, chains = 3){
  datos_sim_2 <- marco_noreste %>% mutate(y = sim_y[iteracion, ])
  muestra_sim <- muestra_part_noreste %>% left_join(datos_sim_2 %>% select(-ln))
  data <- list(k = length(unique(muestra_part_noreste$estrato)), 
               N = nrow(marco_noreste), 
               n = nrow(muestra_sim), est = marco_noreste$estrato_aux, 
             est_muestra = muestra_sim$estrato_aux,
                 votantes_nom = marco_noreste$ln, y = muestra_sim$y,
             votantes_nom_muestra = muestra_sim$ln)
...
muestra_part_noreste <- muestra_part_noreste %>% 
  mutate(ln = ifelse(ln < TOTAL, TOTAL, ln))
data <- list(k = length(unique(muestra_part_noreste$estrato_aux)), 
             N = nrow(marco_noreste), n = nrow(muestra_part_noreste),
             est = marco_noreste$estrato_aux, 
             est_muestra = muestra_part_noreste$estrato_aux,
             votantes_nom = marco_noreste$ln, y = muestra_part_noreste$TOTAL,
             votantes_nom_muestra = muestra_part_noreste$ln)
...
# checar por estrato
dim(sims_comp$y_sim)
m_tbl <- muestra_part_noreste %>% mutate(casilla = 1:dim(sims_comp$y_sim)[2]) 
...
resumen <- posterior_casilla %>% group_by(rep, estrato_aux) %>%
  summarise(votos = sum(votos), total = sum(TOTAL)) %>% 
  group_by(estrato_aux) %>% 
  summarise(q05 = quantile(votos, 0.05), 
            q95 = quantile(votos, 0.95), 
            q50 = quantile(votos, 0.5),
            total = total[1])

resumen <- resumen %>% 
  mutate(estrato_fct = fct_reorder(factor(estrato_aux), total))
ggplot(resumen, aes(x=estrato_fct, y = total, ymin = q05, ymax = q95)) + 
  geom_point() +
  geom_errorbar(colour = "red")

resumen_edo <- posterior_casilla %>% group_by(rep, edo_nombre) %>%
  summarise(votos = sum(votos), total = sum(TOTAL)) %>% 
  group_by(edo_nombre) %>% 
  summarise(q05 = quantile(votos, 0.05), 
            q95 = quantile(votos, 0.95), 
            ...
            total = total[1]) %>% 
  ungroup() %>% 
  mutate(edo_fct = fct_reorder(factor(edo_nombre), total))

ggplot(resumen_edo, aes(x=edo_fct, y = total, ymin = q05, ymax = q95)) + 
  geom_point() +
  geom_errorbar(colour = "red")
```